...
- **Images pending:** v2 went live on 2026-09-25 (`007ecd9`) without the user's images. When they land in `img/`: wire `img/bni-logo.*` into the header and add favicon / apple-touch-icon / og-image tags (spec in `img/README.md`), preview locally, push, verify live. Cache: Cloudflare edge TTL is 5 min (per user), but GitHub Pages sends `cache-control: max-age=600`, so browsers can hold old JS/data for up to 10 min after a release that changes HTML + JS + data together. User declined `?v=` tags.
- **Pushing:** git here has no GitHub credential helper. Push with `git -c credential.helper= -c credential.helper='!gh auth git-credential' push origin main` (uses the user's existing `gh` login for that command only; don't run `gh auth setup-git` without asking, it changes global git config).
